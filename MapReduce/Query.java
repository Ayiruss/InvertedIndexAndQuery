...
public class Query  extends Configured implements Tool {
    ...
    public static void printResult(FileInfo [] resultArray, Path docPath, FileSystem fs)
        ...
        for (FileInfo fileInfo : resultArray) {
           ...
           for (TermOffsets tmOfs: fileInfo.getTermOffsets()) {
                ...
                RemoteIterator<LocatedFileStatus> itr = fs.listFiles(docPath, true);
                while(itr.hasNext()){
                	LocatedFileStatus lfs = itr.next();
                	String[] fileInPath = lfs.getPath().getName().split("/");
                	if(fileInPath[fileInPath.length-1].equals(fileInfo.getFileName())){
                		newPath = lfs.getPath();
                		break;
                	}
                }
      /*          if (list != null) {
                ...
                FSDataInputStream doc = fs.open(new Path(newPath.toString()));
    ...
    public int run(String[] args) throws Exception {
        ...
        URI uri = new URI("s3n://invertedindex-hadoopspark/input");
