...
public class RealTimeSpark
{
    ...
    {
        SparkConf conf = new SparkConf().setMaster("yarn-cluster").setAppName("SparkApp");

        JavaSparkContext sc = new JavaSparkContext(conf);
        // Load the input data, which is a text file read from the command line in the format key file name and content file
        JavaPairRDD<String,String> input = sc.wholeTextFiles( args[0] );
   
        //Split each file line by line and add line number to each line then map it again to the same filename
        ...
        //Split word by word and map it to filename
        ...
        //Swap the key value pair, now word is the key and filename is the value
        ...
        //Get the filename where the search 
